...
#They are represented as Unknown's
```{r}
glimpse(polar_bears)
```
...
polar_bears %>% 
  ...
  geom_bar()+
labs(title= "Sapmpling of Male and Female",
     x="Sex",
     y="Count")
```
...
polar_bears %>% 
 count(capture_date) %>% 
  ...
  geom_col()+
  labs(title="Bears Sampled in Each Year",
       x="Capture Date",
        y="Sex")
```
...
polar_bears %>% 
 count(popn) %>% 
   ggplot(aes(x=popn, y=n, fill=popn))+
  geom_col()+
  labs(title="Bears Fasting",
       x="Population",
        y="SB")
```
