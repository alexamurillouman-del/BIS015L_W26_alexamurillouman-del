...
labs(title= "Sapmpling of Male and Female Bears by Population",
     x="Population",
     y="Sex")
```
...
```{r}
polar_bears_yearly <- polar_bears %>% 
...
polar_bears_yearly %>% 
  ...
  geom_line(size=1)+
  geom_point()+
  labs(title="Bears Sampled by Year for Each Population",
       x="Year",
        y="Count")
```
...
polar_bears %>% 
  ...
  summarize(min_urea_nitrogen=min(urea_nitrogen, na.rm=T),
           med_urea_nitrogen=median(urea_nitrogen, na.rm=T),
...
summarize(min_creatinine=min(creatinine, na.rm=T),
           med_creatinine=median(creatinine, na.rm=T),
...
polar_bears <- polar_bears %>% 
  mutate(fasting= if_else(un_creat_ratio<= 10, "fasting", "not fasting"))
```
...
polar_bears %>%
  ...
  geom_point() +
  labs(
    title = "Number of Fasting Bears by Year and Population",
    x = "Year",
    y = "Count"
  ) +
...
ggmap(map1) +
  ...
  labs(
    x = "Longitude",
    y = "Latitude",
    title = "Locations of Polar Bears by Population"
  )
```
...
server <- function(input, output, session) {
 ...
 ggplot(polar_bears, aes(x = popn, y = un_creat_ratio, fill = .data[[input$fill_var]])) +
      geom_boxplot() +
      labs(
        title = "Range of un_creat_ratio Values for Both Populations",
        x = "Population",
        y = "un_creat_ratio",
        fill = input$fill_var
      )
