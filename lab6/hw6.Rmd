...
```{r}
gabon %>%
  ggplot(aes(x = land_use, fill=land_use)) +
  geom_bar() +
  labs(x = "Land use type", y = "Number of transects")
```

**6. How does vegetation richness compare among land use types?**
```{r}
gabon %>%
  ggplot(aes(x = land_use, y = veg_rich, fill= land_use)) +
  geom_boxplot()
```
...
```{r}
gabon %>%
  ggplot(aes(x = land_use, y = diversity_all_species, fill= land_use)) +
  geom_boxplot()
```
