...
https://github.com/alexamurillouman-del/BIS015L_W26_alexamurillouman-del.git
...
```{r}
anolis %>% 
  ggplot(mapping=aes(x=habitat, y=species)) +
  geom_col()
```
...
```{r}
anolis %>% 
  ggplot(mapping=aes(x=body_length_mm, y=habitat, fill=habitat))+
geom_col()
 ("Relationship between body length and tail length")
```
```{r}
anolis %>% 
  ggplot(mapping=aes(x=tail_length_mm, y=habitat, fill=habitat))+
geom_col()
```
